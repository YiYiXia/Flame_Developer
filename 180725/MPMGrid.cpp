...
MPMGrid::MPMGrid(Vector2d pos, Vector2d dims, Vector2d cells,MPMParticleCloud* object)
{
	obj = object;
	start = pos;
	end = dims;
...
MPMGrid::MPMGrid(Vector2d pos, Vector2d dims, MPMParticleCloud* object)
{
	obj = object;
	start = pos;
	end = dims;
...
void MPMGrid::initializeVelocities()
{
	for (int i = 0; i < obj->size; i++)
	{
		MPMParticle* p = obj->particles[i];
...
void MPMGrid::calculateVolumes() const
{
	for (int i = 0; i<obj->size; i++)
	{
		MPMParticle* p = obj->particles[i];
...
void MPMGrid::gridVelocityUpdate()
{
	for (int i = 0; i<nodes_length; i++)
	{
		MPMGridNode &node = nodes[i];
...
void MPMGrid::ImplicitVelocities()
{
...
void MPMGrid::RecomputeImplicitForces()
{
	//Vector2d temp;
	//int key = 0;
	for (int idx = 0; idx<nodes_length; idx++)
...
void MPMGrid::ParticleCheck()
{
	obj->particles.erase(std::remove_if(obj->particles.begin(), obj->particles.end(), [&](MPMParticle* & p)
	{
		//cout << "check" << endl;
		int co;
...
void MPMGrid::ParametersIn()
{
	ifstream infile;
	infile.open("GridConfig.txt");
	if (!infile.is_open())
	{
...
void MPMGrid::GridCheck()//方程式解法的初始化
{
	active_length = 0;
	active_list.clear();
	for (int y = 0, idx = 0; y < size[1]; y++)
