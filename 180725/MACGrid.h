...
typedef struct MACGridNode
{
	...
	Vector2d position;
	MACNodeType type;
	vector<MPMParticle*> particles;//存放格点中的粒子
	int index;//格点的编号
	int index_x, index_y;
	int s_index;
	MACGridNode()
...
{
public:
	MACGrid(Vector2d pos, Vector2d dims, Vector2d cells);
	MACGrid(Vector2d pos, Vector2d dims, Vector2d cells, MPMParticleCloud* object);
	~MACGrid();
	MACGridNode* nodes;
	...
	void Correct();
	void GridCheck();
	void GetNeighbour(int i, int j, int wl, int wh, int hl, int hh, vector<MPMParticle*> &neighbour);
	void Initial();
	void SolvePressure();
	//void Transport();
	//void Update();
